...
| | 檢索 Recall@3 | 關鍵字正確率 | negative 拒答率 | 數字接地 | 答案 TTFT | TPS |
|---|---|---|---|---|---|---|
| **本系統（RAG）** | **1.000** | **96.8%** | **100%** | **100%** | 0.113 s | 61.7 |
| 同模型、無 RAG 對照 | — | 29.0% | **0%** | **0%** | 0.118 s | 65.0 |

峰值 VRAM ≈ 1.6 GB / 4 GB。兩個模型的完整對照見 [§7.4](#74-結果)。
...
| 項目 | 配置 | 佔用 |
|---|---|---|
| 生成模型權重 | Qwen3-1.7B **Q4_K_M** | **1.11 GB** |
| KV cache | `n_ctx=4096`、`type_k/type_v=q8_0` | **0.232 GB** |
...
|---|---|---|---|---|---|
| **A 預設** | **Qwen3-1.7B Q4_K_M** | **1.11 GB** | **0.232 GB** | **~1.6 GB** | **本專案預設（實測選出）** |
| B | Qwen2.5-3B Q4_K_M | 1.93 GB | 0.075 GB | ~2.3 GB | 只能用 top-3 時較穩、長 context 較省 KV |
| C | Qwen3-4B-Instruct-2507 Q4_K_M | 2.50 GB | — | ~3.1 GB | 未實測 |

Embedding 一律用 bge-m3 Q8_0（0.63 GB，掛 CPU）。

### 3.3 模型選擇：一個被自己的評測推翻的決定

**原始判斷**：RAG 是抽取式任務 —— 答案已經在 context 裡，模型只要讀懂再抄出來，
不需要「記得」規格。所以參數量不是關鍵，指令遵循才是。基於這個論證選了
Qwen2.5-3B（繁中在 3B 級距最強、不會簡繁混寫）。

**實測結果否定了「3B 才夠」這一半**（完整數據見 §7.4）：

| 模型 | top-k | 答案 TTFT | TPS | 關鍵字 | 誤拒 | 數字接地 | 事實/拒答/推理 | 權重+KV |
|---|---|---|---|---|---|---|---|---|
| **Qwen3-1.7B** | **5** | **0.113 s** | **61.7** | 96.8% | 0% | **100%** | 25/25 · 5/5 · 5/6 | **1.34 GB** |
| Qwen2.5-3B | 5 | 0.167 s | 36.0 | 96.8% | 0% | 98.4% | 25/25 · 5/5 · 5/6 | 2.01 GB |

**在 top-5，1.7B 在每一項品質指標上都不輸 3B**，而且：

```
TPS         61.7 vs 36.0     快 1.7 倍
答案 TTFT   0.113 vs 0.167 s  快 1.5 倍
權重 + KV   1.34 vs 2.01 GB   省 0.67 GB
```

兩者甚至答錯的題目**不同**（1.7B 錯 rs02、3B 錯 rs01，都是接孔選擇的推理題），
顯示這不是「一個比較強」，而是兩者都撞到同一類問題的天花板。

**所以預設改為 Qwen3-1.7B + `top_k=5`。** 原始論證只對了一半：
「抽取式任務對參數量不敏感」是對的 —— 對到連 3B 都是多餘的。

**但有兩個必須寫出來的但書：**

1. **1.7B 需要更深的檢索才能達標。** top-3 時它只有 93.5%（3B 是 96.8%），
   誤拒率 6.5%（3B 是 3.2%）。它的容錯空間比較小，靠 top-5 補回來。
   如果延遲預算緊到只能用 top-3，3B 反而是比較穩的選擇。
2. **KV cache 反而變大。** 1.7B 的 KV head 有 8 個、3B 只有 2 個，
   所以 KV cache 是 0.232 GB vs 0.075 GB（見 §3.1）。context 若要拉長到
   8k 以上，這個差距會吃掉權重省下來的空間。

**量化維持 Q4_K_M**：社群共識是 Q4_K_M 以上掉點很小、Q3 以下明顯退步。
本專案沒有實測量化掃描（見 §8），這一項仍是引用而非自證，據實標明。
...
### 7.2 一個差點讓對照組失效的陷阱：Qwen3 的 thinking mode

第一次跑 Qwen3-1.7B 的對照組時，數據看起來好得可疑 ——
關鍵字正確率 100%（贏過 3B 的 96.8%）、TTFT 0.045 s（快 3.7 倍）。

檢查原始輸出才發現，**36 個答案全部包含 `<think>` 區塊**：

```
1.7B 平均答案長度  569 字元    ← 絕大部分是內心獨白
3B   平均答案長度   41 字元

而且獨白是簡體中文：「好的，用户问的是…参考资料里…」
```

Qwen3 是**混合推理模型**，thinking mode 預設開啟。這讓三個指標同時失真：

| 指標 | 失真原因 |
|---|---|
| 關鍵字正確率 | 關鍵字出現在獨白裡，不代表最終答案講對 |
| TTFT | 量到的是 `<think>` 的第一個 token，不是使用者看得到的第一個字 |
| 數字接地 | 未接地的數字全部來自獨白 |

**處理方式**（`config.ModelSpec.thinking_switch` + `llm.strip_thinking`）：

1. 對有 thinking mode 的模型自動附加 Qwen3 的軟開關 `/no_think`
   —— 實測 token 數從 92 降到 15
2. 計分前剝除 `<think>...</think>`，只用使用者看得到的內容評分
3. 新增 **`ttft_answer_s`** 指標：量到「第一個**答案** token」而非第一個 token。
   兩者的差就是 thinking 的成本；非推理模型兩個數字相同

```
$ uv run aorus-rag ask "這台電池多大？" --model qwen3-1.7b
這台電池為 99Wh [2]。
--- retrieval 185.0 ms | TTFT 0.654 s (model 0.469 s, 首個答案 token 0.719 s)
    | 62.0 tok/s decode | 17 tokens | prompt 325 tokens
```

> **這是自建評測最大的風險**：指標會給你一個數字，但不會告訴你它量錯了東西。
> 如果只看彙總表格，這份報告就會宣稱「1.7B 全面勝過 3B」——
> 而那個結論建立在把模型的內心獨白當成答案來評分之上。
...
##### (e) 模型對照：Qwen3-1.7B vs Qwen2.5-3B

同一組 36 題、同一個 retriever、同樣的 prompt。**1.7B 的數字已套用 §7.2 的
thinking 處理**，兩者的 `答案 TTFT` 因此是可比的。

| 模型 | top-k | 答案 TTFT | TPS | 關鍵字 | 拒答 | 誤拒 | 數字接地 | 事實 / 拒答 / 推理 |
|---|---|---|---|---|---|---|---|---|
| **Qwen3-1.7B** | **5** | **0.113 s** | **61.7** | 96.8% | 100% | 0% | **100%** | 25/25 · 5/5 · 5/6 |
| Qwen3-1.7B | 3 | 0.114 s | 61.4 | 93.5% | 100% | 6.5% | 100% | 25/25 · 5/5 · 4/6 |
| Qwen2.5-3B | 5 | 0.167 s | 36.0 | 96.8% | 100% | 0% | 98.4% | 25/25 · 5/5 · 5/6 |
| Qwen2.5-3B | 3 | 0.165 s | 35.9 | 96.8% | 100% | 3.2% | 100% | 25/25 · 5/5 · 5/6 |
| 1.7B no-RAG | — | 0.118 s | 65.0 | 29.0% | **0%** | 0% | **0%** | 6/25 |
| 3B no-RAG | — | 0.115 s | 36.0 | 25.8% | **0%** | 3.2% | **0%** | 3/25 |

**結論（已反映在預設值）**：top-5 時 1.7B 在每一項品質指標上都不輸 3B，
TPS 快 1.7 倍、答案 TTFT 快 1.5 倍、權重加 KV 少 0.67 GB。
兩者答錯的甚至是**不同**的題目（1.7B 錯 rs02、3B 錯 rs01，都是接孔選擇），
代表這不是「誰比較強」，而是兩者撞到同一類問題的天花板。

**1.7B 的代價**：需要更深的檢索。top-3 時掉到 93.5%、誤拒率翻倍到 6.5%。
它的容錯空間比 3B 小，靠 top-5 補回來。

##### (f) `/no_think` 的殘餘成本

即使關掉 thinking，Qwen3 仍會輸出一個**空的** `<think></think>` 區塊：

```
rag_top5:  TTFT 0.045 s  →  答案首字 0.113 s   額外 68 ms
```

**68 ms 是關掉 thinking 之後仍付出的固定成本**，佔 1.7B 答案 TTFT 的 60%。
若沒有 `ttft_answer_s` 這個指標，這段延遲會被算成「1.7B 的 TTFT 只有 0.045 s」，
比 3B 快 3.7 倍 —— 一個看起來很漂亮但錯誤的結論。

另外，軟開關偶爾會被模型回吐到輸出裡（108 個答案中出現 2 次，
如 `提供的規格資料中沒有這項資訊。 /no_output`）。`llm.strip_thinking()`
會一併清除，並有回歸測試覆蓋。
...
| 項目 | 狀態 |
|---|---|
| Qwen3-4B 對照組 | ⛔ 未下載（2.50 GB） |
| 量化掃描 Q4_K_M / Q5_K_M / Q8_0 | ⛔ 未執行 |
| Colab / Kaggle T4 的 `nvidia-smi` VRAM 佐證 | ⛔ 未執行 |

原因據實記錄：本機為 8 GB 統一記憶體。3B 的完整掃描（約 470 次生成）
會把 swap 推到 1.6 GB 並使機器明顯發熱；1.7B 的掃描則全程 swap ≤ 884 MB、
無任何散熱警告。再加測更多模型應移到 Kaggle T4，
**而不是在受污染的環境下硬跑出數字**。

> 兩次評測都在監督腳本下執行（`scripts/` 未收錄，見 commit 紀錄），
> 設三道自動中止防線：CPU 因過熱降頻、swap > 1700 MB、可用記憶體 < 0.8 GB。
> 兩次都未觸發。
...
1. **只有兩個模型的對照，且都在 2B 以下與 3B 級距**。1.7B 與 3B 的完整掃描已完成
   並據此改了預設值，但 4B 未測（見 §7.4(g)），所以「再大是否還有邊際效益」
   仍是未知數 —— 目前只證明了「1.7B 足夠」，沒有證明「更大沒用」。
   量化維度（Q5_K_M / Q8_0）完全未測，Q4_K_M 的選擇仍是引用社群共識。
...
- **補完模型與量化掃描**：4B 以及 Q4_K_M / Q5_K_M / Q8_0 的品質-記憶體-延遲曲線，
  應在 Kaggle T4 上執行以避免本機記憶體壓力污染量測。
