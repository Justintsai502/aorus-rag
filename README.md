...
6. **單一 SKU 範圍**：AM6H 很可能是機型家族頁，實際販售的 BZH / BYH / BXH
   三個 SKU 差在顯示卡（5090 / 5080 / 5070 Ti）。本系統只涵蓋 AM6H 規格表
   本身所載的內容，因此「顯示卡是什麼」會答 RTX 5090。跨 SKU 比較不在範圍內 ——
   正確的支援方式是把機型代號做成 chunk 的 metadata 並讓檢索能依機型過濾，
   而不是把三台機器的規格無標記地混進同一份語料（那正是 §5.2 要防的事）。
