...
# -e: stop on the first failing command; -u: error on unset variables;
# -o pipefail: a pipeline fails if any stage fails.
set -euo pipefail
# Run from the project root regardless of the caller's working directory.
cd "$(dirname "$0")/.."
# AORUS_RAG_MODELS overrides the download directory (default: ./models).
MODELS_DIR="${AORUS_RAG_MODELS:-$PWD/models}"
mkdir -p "$MODELS_DIR"

# Associative arrays (bash 4+): model name -> HuggingFace repo, file name, size.
...
# Default: the two models in use -- Qwen3-1.7B for generation, bge-m3 for embeddings.
DEFAULT=(qwen3-1.7b bge-m3)
# Every model listed above. The e5-small GGUF currently fails to load (see config.py).
ALL=(qwen2.5-3b qwen3-1.7b qwen3-4b e5-small bge-m3)

# No argument = default set; "all" = every model; anything else = model names.
case "${1:-default}" in
...
# For each requested model: validate the name, skip it if present, otherwise download.
for name in "${TARGETS[@]}"; do
  # ${REPO[$name]:-} expands to empty for an unknown name instead of tripping set -u.
  if [[ -z "${REPO[$name]:-}" ]]; then
  ...
  # -f: fail on HTTP errors instead of saving the error page as the model file
  # -L: follow redirects (HuggingFace redirects to its CDN)
...
echo
# List downloaded model sizes; "|| true" keeps an empty directory from failing the script.
